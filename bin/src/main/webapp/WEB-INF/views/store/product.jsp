	...
	<div class="productContent">
		...
		<div class="productContext">
			...
			<div>
				...
				<FORM id="test" action="<c:url value='addToCart' />" method="POST">
					購買數量: <select name="qty" class="selectBuyQty">
		...
		<script>
			...
			/* 
			var butQty = $('.selectBuyQty').val();

			var productStock1 = $('.stock1').attr('value');
			console.log(productStock1);
			
			$('.selectBuyQty').change(function() {
				var butQty = $('.selectBuyQty').val();
				var productStock = $('#product-stock').attr('value');
				if (butQty > productStock) {
					console.log(butQty);
					console.log(productStock);
					alert("超過庫存量");
				
					$('.selectBuyQty')[0].selectedIndex=$($('#product-stock').attr('value')-1;;
				}
			}); */
